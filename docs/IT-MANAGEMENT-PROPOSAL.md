...
* **Declarative Bare-Metal & Cloud IaC via OpenTofu:** Where physical hypervisor slices, virtual networks, or geodistributed cloud instances require provisioning, Ansible calls **OpenTofu** (the open-source, sovereign fork of Terraform). OpenTofu state files are persisted in Ceph/MinIO S3 compliance storage equipped with dedicated state-governance controls:
  * **S3 Native State Locking:** For native Ceph/MinIO S3 backends, state locking is enforced directly via native S3 lockfiles (`use_lockfile = true` using S3 conditional write primitives). In AWS or hybrid cloud environments utilizing a DynamoDB lock backend, `dynamodb_table` binds to a dedicated DynamoDB table endpoint. During migration phases, both mechanisms operate in tandem across respective cloud and on-premise backends.
