...
def test_staging_retention_is_bounded_and_automated() -> None:
    """Verify active quarantine directories cannot accumulate indefinitely."""
    lifecycle = _section("### 3.2", "### 3.3")

    assert all(
        path in lifecycle
        for path in ("/data/staging/raw/", "/data/staging/verify/", "/data/staging/failed/")
    )
    assert "14-day retention rules" in lifecycle
    assert "automated NiFi cleanup processors" in lifecycle
...
def test_webgpu_failure_has_cpu_and_server_fallbacks() -> None:
    """Verify unsupported features and allocation failures retain safe fallbacks."""
    capabilities = _section("### 4.4", "### 4.5")

    assert "features are unavailable or fail allocation" in capabilities
    assert "Wasm CPU execution using standard SIMD" in capabilities
    assert "server-side Apache NiFi validation" in capabilities
