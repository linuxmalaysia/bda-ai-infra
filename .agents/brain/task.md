...
0. **BDA Data Plane Upgrade Technical Proposal (`docs/proposals/nre-bda-pipeline-upgrade.md`)**:
   ...
   - Addressed all PR review requirements: untreated raw file persistence into `/data/staging/raw/`, advisory client metrics, 14-day retention across `/data/staging/failed/` and `/data/staging/rejected/`, single-use JWT replay protection (record ID, tenant, approver, audience, nonce, 5-min TTL), sanitized IP addresses and hostnames, and `HEX_RAW_64_BYTE` 128 uppercase hex character signature specification.
