...
- **Apache APISIX:** Instrumented using the native APISIX `opentelemetry` plugin for distributed HTTP tracing (propagating W3C `traceparent` headers) and `prometheus` metrics plugin capturing request latency, HTTP status codes, and bandwidth.
