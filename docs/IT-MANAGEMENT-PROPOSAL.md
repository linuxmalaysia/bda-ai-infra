...
* **Declarative Bare-Metal & Cloud IaC via OpenTofu:** Where physical hypervisor slices, virtual networks, or geodistributed cloud instances require provisioning, Ansible calls **OpenTofu** (the open-source, sovereign fork of Terraform). OpenTofu state files are persisted in Ceph/MinIO S3 compliance storage equipped with dedicated state-governance controls:
  * **S3 Native State Locking:** Ceph/MinIO S3 backends enforce state locking via native S3 lockfiles (`use_lockfile = true`), requiring an S3-compatible object store supporting OpenTofu S3 conditional writes using the `If-None-Match` header. In AWS or hybrid cloud environments configured with DynamoDB state locking, `dynamodb_table` identifies an existing DynamoDB table, `endpoints = { dynamodb = ... }` selects its custom endpoint if applicable, and the DynamoDB table requires a `LockID` String partition key. Both locking mechanisms do not cross-coordinate across backends; each locking mechanism applies strictly and independently to its configured backend state storage.
