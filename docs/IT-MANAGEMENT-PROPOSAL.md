...
* **Hermetic Python Tooling via `uv`:** To prevent dependency drift and environment pollution, all Python runtimes, Ansible dependencies, and linting suites are executed via `uv` (`uv run --locked ansible-playbook`). This ensures sub-second virtual environment initialisation, strict lockfile reproducibility, and zero external dependency contamination on the Proxmox host hypervisors.
* **Complete Operational Auditability via ARA (Ansible Records Ansible):** Every playbook execution triggered by human operators or agentic pipelines is intercepted and recorded by **ARA Records Ansible**. Sensitive tasks and credential-bearing parameters (such as Vault secrets or private keys) enforce strict `no_log: true` directives to prevent secret leakage prior to persistence. ARA records execution playbooks, task outcomes, non-sensitive parameters, and host-level diffs into a dedicated SQLite or PostgreSQL audit database protected by least-privilege role-based access controls (RBAC), providing an auditable, timestamped ledger of every state change across the data plane.
* **Declarative Bare-Metal & Cloud IaC via OpenTofu:** Where physical hypervisor slices, virtual networks, or geodistributed cloud instances require provisioning, Ansible calls **OpenTofu** (the open-source, sovereign fork of Terraform). OpenTofu state files are persisted in Ceph/MinIO S3 compliance storage equipped with dedicated state-governance controls:
  * **S3 State Locking:** Prevents concurrent writes and state race conditions by enforcing DynamoDB/S3-native lock tables during execution.
