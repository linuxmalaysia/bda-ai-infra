...
* **Standardised Ingress & Egress:** The Fusio API Server exposes unified endpoints utilising REST and gRPC over HTTPS (ports 8080 and 443). Data contracts are strictly enforced at the perimeter using OpenAPI 3.0 and TypeSchema specifications, automatically rejecting payloads that violate structural or type boundaries before they enter the processing queues.
...
* **Row and Column Level Security:** The Fusio API and MCP gateways dynamically inject session context parameters (e.g., `SELECT set_config('app.current_user_role', $1, true)` executed within each request transaction, binding `$1` to the authenticated principal) into the PostgreSQL transactional session. Native Row-Level Security (RLS) policies within PostgreSQL automatically filter out unauthorised records at the engine level, guaranteeing that API consumers and AI agents only access their explicitly permitted data slices.
