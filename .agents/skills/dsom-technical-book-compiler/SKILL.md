...
23. **Bidirectional WSL-to-Host Output Synchronization Invariant:** When compiling technical handbooks, publication documents, or generating analytical charts in the native Linux WSL environment (`/home/haris/kerja/myssm_v4-fusio-api`), the AI agent MUST systematically copy all newly generated publication assets (`build/book/*.html`, `*.pdf`, `*.epub`, `master_book.md`, and `build/book/assets/*`) back to the Windows host mount path (`/mnt/d/Users/LinuxMalaysia/Projects/myssm_v4-fusio-api/build/book/`). Never report a task as complete if the generated artifacts exist only in WSL and not in the Windows host directory, preventing user-facing desynchronisation and phantom missing chapter errors.
